...
function cargarHistorial() {
  try {
    const raw = localStorage.getItem('cayotest_historial');
    if (raw) {
      const arr = JSON.parse(raw);
      const changed = normalizeHistorial(arr);
      if (changed) localStorage.setItem('cayotest_historial', JSON.stringify(arr));
      return arr;
    }
  } catch (_) {}
  return [];
...
function cargarAuditorias() {
  try {
    const raw = localStorage.getItem('cayotest_auditorias');
...
function cargarIngresos() {
  try { return JSON.parse(localStorage.getItem('cayotest_ingresos') || '[]'); }
  catch (_) { return []; }
}
...
function cargarArqueos() {
  try { return JSON.parse(localStorage.getItem('cayotest_arqueos') || '[]'); }
  catch (_) { return []; }
}
let arqueos = cargarArqueos();
function guardarArqueos() {
  localStorage.setItem('cayotest_arqueos', JSON.stringify(arqueos));
  pushToCloud();
}
...
function cargarArchivo() {
  try { return JSON.parse(localStorage.getItem('cayotest_archivo') || '[]'); }
  catch (_) { return []; }
}
let archivo = cargarArchivo();
let cierreTemporada = parseInt(localStorage.getItem('cayotest_cierre') || '0', 10) || 0;
function guardarArchivo() {
  localStorage.setItem('cayotest_archivo', JSON.stringify(archivo));
  localStorage.setItem('cayotest_cierre', String(cierreTemporada));
}

// Mostrar u ocultar pesos argentinos en la parte de ventas (vendiendo en Uruguay)
let mostrarARS = localStorage.getItem('cayotest_mostrar_ars') === '1';
...
function guardarPedidos() {
  localStorage.setItem('cayotest_pedidos', JSON.stringify(pedidos));
}
...
const GAS_URL_KEY = 'cayotest_gas_url';
...
document.getElementById('btn-lock').addEventListener('click', () => {
  if (rol !== 'lectura') {
    if (localStorage.getItem('cayotest_rol') || localStorage.getItem('cayotest_admin') === '1') {
      if (confirm('¿Querés que este dispositivo deje de recordar el PIN?')) {
        localStorage.removeItem('cayotest_rol');
        localStorage.removeItem('cayotest_admin');
      }
    }
    setRol('lectura');
    return;
...
function confirmarPin() {
  ...
  if (nuevoRol) {
    ...
    if (document.getElementById('pin-recordar')?.checked) {
      localStorage.setItem('cayotest_rol', nuevoRol);
    }
    setRol(nuevoRol);
  } else {
...
const rolGuardado = localStorage.getItem('cayotest_rol')
  || (localStorage.getItem('cayotest_admin') === '1' ? 'admin' : 'lectura');
...
document.getElementById('btn-cerrar-temporada').addEventListener('click', () => {
  ...
  localStorage.setItem('cayotest_historial',  '[]');
  localStorage.setItem('cayotest_pedidos',    '[]');
  localStorage.setItem('cayotest_ingresos',   '[]');
  localStorage.setItem('cayotest_arqueos',    '[]');
  localStorage.setItem('cayotest_auditorias', '[]');
  guardarArchivo();
...
document.getElementById('config-mostrar-ars').addEventListener('change', function() {
  ...
  localStorage.setItem('cayotest_mostrar_ars', mostrarARS ? '1' : '0');
  aplicarMostrarARS();
  renderTodo();
});
